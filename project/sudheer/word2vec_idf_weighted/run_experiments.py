#!/usr/bin/env python3
"""
Run baseline TF-IDF retrieval and an IDF-weighted Word2Vec centroid backbone
for the Cranfield collection.

The method trains Word2Vec on the stopword-removed Cranfield document tokens,
weights each token vector by TF * IDF, averages the weighted vectors, and
ranks documents by cosine similarity in the dense embedding space.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run IDF-weighted Word2Vec centroid experiments.")
    ...
    term_weights = baseline_index.idf_by_term()
    ...
    save_primary_sweep_plot(
        ...
        title="Best Result Per Word2Vec Embedding Size (IDF-weighted)",
    )
    ...
    save_metric_plot(
        ...
        method_metrics,
        "IDF-weighted Word2Vec Backbone Metrics",
    )
    ...
    save_overlay_plot(
        ...
        method_labels,
        ordered_keys,
        "Baseline vs Best IDF-weighted Word2Vec Models by Embedding Size",
    )
    save_all_tuned_combinations_overlay(
        ...
        sweep_results,
        best_config,
        "Baseline vs All Tuned IDF-weighted Word2Vec Configurations",
    )
    ...
    build_experiment_report(
        ...
        title="IDF-weighted Word2Vec Backbone Experiment Report",
        ...
        method_summary_lines=[
            ...
            "The proposed backbone still uses Word2Vec token embeddings, but instead of a plain centroid it weights "
            "each token vector by TF * IDF before averaging.",
            "This gives more influence to technically informative terms and reduces the dominance of very common "
            "words, while still keeping the retrieval model simple and fast.",
        ],
        hypothesis_lines=[
            "Compared with plain averaging, IDF-weighted centroids should preserve more query intent because rare "
            "technical terms contribute more strongly to the final dense representation.",
            "The method still compresses each item into a single vector, so it is expected to help semantic matching "
            "without fully solving ambiguity, phrase structure, or local context problems.",
        ],
        limitations_lines=[
        ...
        interpretation_lines=[
            "This method is a stronger first Word2Vec backbone than plain averaging because it respects corpus-level "
            "term importance. If a query contains a few decisive technical words, weighting helps those words shape "
            "the centroid instead of being diluted by generic context terms.",
            "Its limitation is structural rather than lexical: the representation is still one dense vector per item, "
            "so compositional detail, token order, and explicit local matching remain weak compared with lexical or hybrid models.",
        ],
        citations=[
            (
