...
LSA applies Singular Value Decomposition to the TF-IDF weighted document-term matrix, discovering latent semantic relationships through matrix factorization. Given a matrix M where m is document count and n is vocabulary size, SVD decomposes:

M = U * Sigma * V^T

where:
- U contains left singular vectors (document space)
- Sigma is diagonal singular values matrix
- V contains right singular vectors (term space)
...
q_latent = q_term * V_k * Sigma_k^(-1)

where q_term is the query's TF-IDF vector. This ensures queries inhabit the same semantic space as documents.

#### Ranking via Latent Similarity

After L2 normalization, similarity is computed via dot product:

score(q, d) = q_latent * d_latent^T
...
|-----------|-------|-----------|
| sublinear_tf | True | Reduce high-frequency term dominance: tf = 1 + log(raw_tf) |
| max_df | 0.95 | Filter ubiquitous terms appearing in >95% of documents |
| min_df | 2 | Remove singleton terms (potential typos) |
| norm | l2 | L2 normalization ensuring comparable similarity scores |
...
TruncatedSVD is applied to sparse TF-IDF matrix with automatic component adjustment.
...
For each query, similarity matrix computed and documents ranked by similarity scores in descending order.

### II.3 Hyperparameter Tuning

#### Dimensionality Sweep: Finding Optimal k

Systematic sweep across k values:

| Components | Precision@10 | Recall@10 | MAP@10 | nDCG@10 | MRR@10 |
|-----------|---|---|---|---|---|
| 100 | 0.2924 | 0.4256 | 0.2978 | 0.4689 | 0.7446 |
| 150 | 0.3053 | 0.4389 | 0.3143 | 0.4862 | 0.7589 |
| 200 | 0.3185 | 0.4561 | 0.3445 | 0.5058 | 0.7713 |
| **250** | **0.3284** | **0.4681** | **0.3703** | **0.5210** | **0.7804** |
| 300 | 0.3218 | 0.4613 | 0.3689 | 0.5226 | 0.7796 |
| 400 | 0.3147 | 0.4525 | 0.3612 | 0.5154 | 0.7851 |

**Finding**: 250 components provides optimal balance across all metrics. Higher dimensions show marginal gains in specific metrics but overall degradation. The 250-component model explains 55.34% of total variance in term-document space.
...
**Document Frequency Bounds**: max_df=0.95 and min_df=2 provide optimal balance of vocabulary richness with noise reduction.
...
- TF-IDF: AP@10 = 0.287 (1/5 relevant, first: rank 5)
- LSA: AP@10 = 0.756 (3/5 relevant, first: rank 1)
- Key insight: LSA learned that "interference effects," "tunnel blockage," "mounting position" form a distinct latent topic.
...
ESA treats documents as explicitly-defined semantic concepts. Rather than discovering abstract latent factors, ESA maps queries into a concrete concept space and ranks documents by concept overlap. Our implementation treats Cranfield documents as concepts (self-concept setting).

#### Mathematical Formulation

**Stage 1: Concept Space Construction**

Document-concept matrix C where:
C_ij = cosine_similarity(tfidf(d_i), tfidf(c_j))

For Cranfield: C is 1398 x 1398 symmetric sparse matrix.

**Stage 2: Query-to-Concept Projection**

Concept activation vector for query q:
s_j = cosine_similarity(tfidf(q), tfidf(c_j)) for all j

**Stage 3: Concept Pruning**

Retain only top-k concepts above minimum threshold with efficient argpartition (O(n) vs O(n log n)).

**Stage 4: Document Ranking via Concept Overlap**

Final document score:
score(q, d) = sum over active concepts of C_dj * s_j / ||s_active||_2

### III.2 Implementation Pipeline

**Stage 1: TF-IDF Vectorization with Bigram Support**

ESA uses richer feature representation including bigrams:
...
| vocabulary | 3,406 | **81,756** | 24x richer feature set |
...
**Stage 2-5**: Concept-Document Similarity Matrix construction, Query projection, Pruning via argpartition, Document Ranking via sparse matrix multiplication.
...
**Finding**: 25 concepts optimal. Fewer concepts (5-10) lose signal; more concepts (30+) add noise.
...
Bigrams are critical because technical multi-word concepts are preserved as atomic units, reducing polysemy and improving discrimination.
...
- TF-IDF: AP@10 = 0.356, ESA: AP@10 = 0.587
- ESA surfaces strongest semantic match
...
- TF-IDF/LSA/Hybrid: FAILED (first relevant >10)
- ESA: AP@10 = 0.214 (recovered!)
- Bigrams captured "heat transfer analysis" as atomic concept

**Query 155**: "technical report on measurement of ablation during flight"
- TF-IDF: 0.276, LSA: 0.339, ESA: 0.500
- Bigrams captured "flight ablation," "in-flight measurement"
...
4. Vocabulary sparsity: Query-document term overlap minimal but conceptual proximity high
...
| Metric | TF-IDF Baseline | LSA | ESA | Hybrid |
|--------|-----------------|-----|-----|--------|
...
| Query Projection | q * V_k * Sigma_k^(-1) | Similarity to 1398 concepts |
...
ESA is ~6x slower per query due to larger feature set and full concept activation computation.
...
ESA requires 4x more memory due to richer feature set.

### IV.4 Method Selection Guidance

**When to Use LSA**:
1. General-purpose semantic retrieval across diverse queries
2. Need for computational efficiency (2x faster per query)
...
**When to Use Hybrid (LSA + TF-IDF)** - BEST OVERALL CHOICE:
1. Seeking best overall performance and robustness
2. Lexical evidence important (proper nouns, exact phrases)
3. Can afford balanced computational cost
4. Want to combine learned latent structure with explicit lexical signals
5. Production systems requiring diverse query handling
...
hybrid_score(q, d) = 0.2 * tfidf_score(q, d) + 0.8 * lsa_score(q, d)
...
**Performance**: Hybrid essentially matches LSA while providing additional robustness.
...
1. **Latent > Explicit**: LSA's learned 250-dimensional topic space outperforms ESA's explicit 1398-document concept space, indicating learned dimensionality reduction captures semantic structure more effectively than explicit document similarity.
...
3. **Feature Richness Trade-off**: ESA's 24x larger feature set (81k vs 3.4k) provides occasional gains but adds computational cost without overall improvement.
...
**Collection**: Cranfield Benchmark (1398 documents, 225 queries)
**Evaluation Framework**: Precision, Recall, F-score, MAP, nDCG, MRR at k=1-10
**Total Configurations Evaluated**: 288 ESA configurations, 6 LSA dimensionality settings
**Best Overall Performance**: Hybrid (LSA + TF-IDF) with 21.9% MAP improvement
